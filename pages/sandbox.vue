...
<template>
  <div
    class="min-h-[calc(100vh-48px)] bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-200"
  >
    <!-- Gradient overlays for depth -->
    <div class="fixed inset-0 pointer-events-none">
      <div
        class="absolute top-0 left-[12%] w-96 h-96 bg-teal-500/10 rounded-full blur-3xl"
      ></div>
      <div
        class="absolute top-20 right-[12%] w-80 h-80 bg-orange-500/10 rounded-full blur-3xl"
      ></div>
    </div>

    <main class="relative flex flex-col h-[calc(100vh-48px)]">
      <section
        class="flex flex-col gap-3 p-4 md:p-5 flex-1 min-h-0 bg-slate-900/40 backdrop-blur-xs"
      >
        <!-- Toolbar -->
          ...
          <div class="flex items-center gap-2">
            ...
            >
              <span
                ...
                >Runtime</span
              >
              <span id="runtimeStatus" class="text-xs font-mono text-slate-200"
                >Idle</span
              >
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 flex-1 min-h-0">
          <!-- Code editor card -->
          <div
            class="md:col-span-2 flex flex-col border border-slate-700 rounded-2xl bg-slate-900/50 p-3 backdrop-blur-sm min-h-0"
          >
            <div class="flex items-baseline justify-between mb-3">
              ...
              <span class="text-xs text-slate-400"
                >Monaco Editor (Dracula)</span
              >
            </div>
            <div
              id="codeEditor"
              class="flex-1 rounded-xl overflow-hidden bg-slate-950 border border-slate-800"
          ...
          <div class="flex flex-col gap-3 min-h-0 overflow-y-auto">
            <!-- Execution card -->
            <div
              class="border border-slate-700 rounded-2xl bg-slate-900/30 p-3 backdrop-blur-sm flex-shrink-0"
            >
            ...
            >
              <div class="flex items-center justify-between mb-2">
                <div>
                  <span
                    class="text-xs font-bold uppercase tracking-wide text-slate-400 block"
                    >Console Output</span
                  >
              ...
              <pre
                id="consoleOutput"
                class="h-24 w-full bg-slate-950 border border-slate-800 rounded-lg p-2 font-mono text-xs text-slate-200 overflow-auto whitespace-pre-wrap break-words"
              ></pre>
            </div>
            ...
            >
              ...
              <pre
                id="errorOutput"
                class="h-24 w-full bg-slate-950 border border-slate-800 rounded-lg p-2 font-mono text-xs text-red-400 overflow-auto whitespace-pre-wrap break-words"
              ></pre>
            </div>
