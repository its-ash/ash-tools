 ...
 * @returns {Array<any>}
 */
export function unzip_files(zip_bytes) {
    const ret = wasm.unzip_files(zip_bytes);
    if (ret[2]) {
 ...
 * @returns {Uint8Array}
 */
export function zip_files(names, contents) {
    const ret = wasm.zip_files(names, contents);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
